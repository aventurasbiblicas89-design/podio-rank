(function(){
  ...
  el.target = '_blank';
  el.rel = 'noopener';
  el.style.cssText = 'display:inline-flex;align-items:center;gap:6px;background:#111;color:#fff;padding:8px 14px;border-radius:999px;font-family:sans-serif;font-size:13px;font-weight:700;text-decoration:none;';
  ...
  function atualizar(){
    fetch('https://www.podiorank.com.br/api/badge?categoria=' + encodeURIComponent(categoria) + '&id=' + encodeURIComponent(id))
      ...
      .then(function(d){
        if(d.posicao){
          document.getElementById('podio-badge-text').innerText = '#' + d.posicao + ' no Podio Rank';
        }
      }).catch(function(){});
  }
  atualizar();
